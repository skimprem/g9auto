...
# def get_absolute_values(input_path):
#     total = pd.DataFrame()
#     for path_to_file in input_path:
#         result = read_project(path_to_file)
#         total = pd.concat([total, result], ignore_index=True)
#     # total.columns = pd.MultiIndex.from_tuples(total.columns, names=['Groups', 'Values', 'Units'])
#     return total
